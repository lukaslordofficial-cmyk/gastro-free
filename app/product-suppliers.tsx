...
export default function ProductSuppliersScreen() {
    ...
    try {
      const [prodRes, catsRes] = await Promise.all([
        supabase
          ...
          .select('id, name, category_id, quantity, unit, min_quantity, portion_size, safety_buffer_percent, is_combo_polprodukt, unit_weight_volume, weight_volume_unit')
          .eq('id', productId)
          .single(),
        supabase
  ...
  async function handleSaveEdit() {
    ...
    try {
      ...
      if (res.error && /safety_buffer_percent|unit_weight_volume|weight_volume_unit/.test(res.error.message ?? '')) {
        const { safety_buffer_percent, unit_weight_volume, weight_volume_unit, ...fallback } = payload;
  ...
  return (
    <SafeAreaView style={s.safe} edges={['top']}>
      {/* Header */}
      <View style={s.header}>
        <TouchableOpacity onPress={() => router.back()} style={s.backBtn} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }} data-testid="product-back">
          <ArrowLeft size={22} color={Colors.textPrimary} strokeWidth={2} />
        </TouchableOpacity>
        <View style={s.headerText}>
          <Text style={s.headerTitle} numberOfLines={1}>{productName ?? 'Produkt'}</Text>
      ...
      <View style={s.tabsRow}>
        <TouchableOpacity
          style={[s.tabBtn, tab === 'suppliers' && s.tabBtnActive]}
          onPress={() => setTab('suppliers')}
          activeOpacity={0.85}
          data-testid="product-tab-suppliers"
        >
          <Truck size={14} color={tab === 'suppliers' ? Colors.accent : Colors.textSecondary} strokeWidth={2} />
          <Text style={[s.tabText, tab === 'suppliers' && s.tabTextActive]}>Dostawcy</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[s.tabBtn, tab === 'edit' && s.tabBtnActive]}
          onPress={() => setTab('edit')}
          activeOpacity={0.85}
          data-testid="product-tab-edit"
        >
          <Pencil size={14} color={tab === 'edit' ? Colors.accent : Colors.textSecondary} strokeWidth={2} />
          <Text style={[s.tabText, tab === 'edit' && s.tabTextActive]}>Edytuj produkt</Text>
        </TouchableOpacity>
      </View>
        ...
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          {loadingEdit ? (
            ...
            <ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              <Text style={s.formSection}>Podstawowe dane</Text>

              <View style={s.field}>
                <Text style={s.label}>Nazwa produktu <Text style={s.req}>*</Text></Text>
                <TextInput
                  style={s.input}
                  value={form.name}
              ...
              <View style={s.field}>
                ...
                {categories.length === 0 ? (
                  <Text style={s.fieldHint}>Brak kategorii w bazie</Text>
                ) : (
                  <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.pillRow}>
                    {categories.map((cat) => {
                      ...
                      return (
                        <TouchableOpacity
                          key={cat.id}
                          style={[s.pill, active && { backgroundColor: cat.color, borderColor: cat.color }]}
                        ...
                        >
                          ...
                          <Text style={[s.pillText, active && { color: Colors.white, fontWeight: '700' }]}>{cat.name}</Text>
                        </TouchableOpacity>
                      );
                    })}
                  </ScrollView>
              ...
              <Text style={s.formSection}>Stan magazynowy</Text>

              <View style={s.fieldRow}>
                <View style={[s.field, { flex: 1 }]}>
                  <Text style={s.label}>Aktualna ilość <Text style={s.req}>*</Text></Text>
                  <TextInput
                    style={s.input}
                    value={form.quantity}
                ...
                <View style={[s.field, { flex: 1 }]}>
                  <Text style={s.label}>Próg krytyczny <Text style={s.req}>*</Text></Text>
                  <TextInput
                    style={s.input}
                ...
                <View style={s.unitRow}>
                  {UNIT_OPTIONS.map((u) => {
                    ...
                    return (
                      ...
                      >
                        <Text style={[s.unitBtnText, active && s.unitBtnTextActive]}>{u}</Text>
                      </TouchableOpacity>
                    );
                  })}
                </View>
              ...
              {(form.unit === 'szt' || form.unit === 'op') && (
                <View style={s.field}>
                  <Text style={s.label}>Waga/objętość jednej {form.unit === 'op' ? 'op.' : 'szt.'}</Text>
                  <View style={s.wvRow}>
                    <TextInput
                    ...
                    />
                    <View style={s.wvToggle}>
                      {(['g', 'ml'] as const).map((u) => {
                        const active = form.weightVolumeUnit === u;
                        return (
                          <TouchableOpacity
                            key={u}
                            style={[s.wvToggleBtn, active && s.wvToggleBtnActive]}
                            onPress={() => setForm((f) => ({ ...f, weightVolumeUnit: u }))}
                            activeOpacity={0.7}
                            testID={`edit-product-wv-unit-${u}`}
                          >
                            <Text style={[s.wvToggleText, active && s.wvToggleTextActive]}>{u}</Text>
                          </TouchableOpacity>
                        );
                      })}
                    </View>
              ...
              <View style={s.field}>
                <View style={s.yieldHeaderRow}>
                  <UtensilsCrossed size={15} color={Colors.accent} strokeWidth={2} />
                  <Text style={s.yieldTitle}>Dostępność w menu</Text>
                  <TouchableOpacity
                    onPress={loadYield}
                    hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
                    style={s.yieldRefresh}
                    testID="yield-refresh"
                  >
                    <RefreshCw size={13} color={Colors.textSecondary} strokeWidth={2} />
                  </TouchableOpacity>
                </View>
                <Text style={s.yieldHint}>
                ...
                {loadingYield ? (
                  <View style={s.yieldLoading}>
                    ...
                    <Text style={s.yieldLoadingText}>Przeliczam porcje…</Text>
                  </View>
                ) : !yieldData || yieldData.dishes.length === 0 ? (
                  <View style={s.yieldEmpty}>
                    <Text style={s.yieldEmptyText}>
                  ...
                  <View style={s.yieldList} testID="yield-list">
                    {yieldData.dishes.map((d) => {
                      ...
                      return (
                        <View key={d.menu_item_id} style={s.yieldRow} testID={`yield-dish-${d.menu_item_id}`}>
                          <View style={{ flex: 1 }}>
                            <Text style={s.yieldDish} numberOfLines={1}>{d.dish_name}</Text>
                            <Text style={s.yieldRecipe}>{d.per_portion_qty} {d.unit} / porcję</Text>
                          </View>
                          {d.convertible ? (
                            <View style={[s.yieldBadge, critical && s.yieldBadgeCritical]}>
                              <Text style={[s.yieldBadgeNum, critical && s.yieldBadgeNumCritical]}>{d.portions}</Text>
                              <Text style={[s.yieldBadgeLabel, critical && s.yieldBadgeNumCritical]}>porcji</Text>
                            </View>
                          ) : (
                            <View style={s.yieldBadgeNa}>
                              <Text style={s.yieldBadgeNaText}>jedn.{'\n'}niezgodne</Text>
                            </View>
                          )}
                        </View>
                      );
              ...
              <View style={s.field}>
                <Text style={s.label}>Bufor bezpieczeństwa (%)</Text>
                <TextInput
                  style={s.input}
                ...
                />
                <View style={s.bufferInfoBox}>
                  <Text style={s.bufferInfoText}>
                    💡 <Text style={s.bufferInfoBold}>Bufor bezpieczeństwa</Text> – Zapas na niezgłoszone straty i ubytki. Ostrzeżenie o braku towaru włączy się o tyle % wcześniej.
                  </Text>
                </View>
              </View>

              <TouchableOpacity
                style={[s.saveBtn, (saving || savedFlash) && { opacity: 0.85 }]}
              ...
              >
                {savedFlash ? (
                  ...
                  <>
                    ...
                    <Text style={s.saveBtnText}>{saving ? 'Zapisywanie...' : 'Zapisz zmiany'}</Text>
                  </>
                )}
              </TouchableOpacity>
