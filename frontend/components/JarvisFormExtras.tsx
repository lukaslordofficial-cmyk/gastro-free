...
export function CriticalOrderEditor({
  ...
  const toggle = (name: string) => {
    ...
    const withoutAll = selected.filter((c) => c !== 'all');
    if (withoutAll.some((c) => c.toLowerCase() === name.toLowerCase())) {
      patch({ categories: withoutAll.filter((c) => c.toLowerCase() !== name.toLowerCase()) });
    } else {
